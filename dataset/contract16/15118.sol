...
if (_amountTokens == uint256(0)) delete offers[msg.sender][_buyer];
else offers[msg.sender][_buyer] = Offer(_amountTokens, _priceWeiPerToken);
}
require (token.allowance(_seller, address(this)) >= _amountTokens);
require(_amountTokens == offers[_seller][msg.sender].amountTokens);
require (_amountTokens.mul(offers[_seller][msg.sender].priceWeiPerToken) == msg.value);
