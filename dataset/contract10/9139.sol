...
bytes32 queryId = oraclize_query(0, "URL", "json(https:
gasprice = _gasprice;
queryIdToIsEthPrice[queryId] = true;
}
}
function scheduleFight() public payable{
require(now > initGameAt && m_Gladiethers.getQueueLenght() > 1 && m_Gladiethers.getGladiatorPower(msg.sender) >= 10 finney && m_Gladiethers.getGladiatorCooldown(msg.sender) != 9999999999999);
