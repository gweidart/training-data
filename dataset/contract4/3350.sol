...
RatInterfaceForForwarder private constant RatKingCorp =
RatInterfaceForForwarder(0xe7c3101745b3dd71228006084dccb619340f8390);
RatBookInterface private constant RatBook =
RatBookInterface(0xf5c4a2c3a92a8450899d064074cec29a9756c95d);
string public constant name = "RatScam Round #1";
string public constant symbol = "RS1";
uint256 private rndGap_ = 0;
uint256 private constant rndInit_ = 1 hours;
uint256 private constant rndInc_ = 30 seconds;
uint256 private constant rndMax_ = 24 hours;
...
(_now <= round_.end || (_now > round_.end && round_.plyr == 0))
)
return (
(round_.keys.add(1000000000000000000)).ethRec(
1000000000000000000
)
);
...
uint256 _now = now;
if (_now < round_.end)
if (_now > round_.strt + rndGap_) return ((round_.end).sub(_now));
else return ((round_.strt + rndGap_).sub(_now));
else return (0);
}
...
(plyr_[_pID].gen).add(
getPlayerVaultsHelper(_pID).sub(plyrRnds_[_pID].mask)
),
plyr_[_pID].aff
);
} else {
...
(round_.mask).add(
(
(((round_.pot).mul(potSplit_)) / 100).mul(
1000000000000000000
)
) / (round_.keys)
)
).mul(plyrRnds_[_pID].keys)
) / 1000000000000000000)
);
}
...
if (
round_.eth < 100000000000000000000 &&
plyrRnds_[_pID].eth.add(_eth) > 10000000000000000000
) {
uint256 _availableLimit = (10000000000000000000).sub(
plyrRnds_[_pID].eth
);
...
updateTimer(_keys);
if (round_.plyr != _pID) round_.plyr = _pID;
...
airDropPot_ = (airDropPot_).sub(_prize);
_eventData_
.compressedData += 100000000000000000000000000000000;
}
_eventData_
...
function calcUnMaskedEarnings(uint256 _pID) private view returns (uint256) {
return (
(((round_.mask).mul(plyrRnds_[_pID].keys)) / (1000000000000000000))
.sub(plyrRnds_[_pID].mask)
);
}
...
require(
msg.sender == address(RatBook),
"only RatBook can call this function"
);
...
if (
!address(RatKingCorp).call.value(_com)(
...
function updateGenVault(uint256 _pID) private {
...
emit RSEvents.onAffiliatePayout(
_affID,
plyr_[_affID].addr,
plyr_[_affID].name,
_pID,
...
plyrRnds_[_pID].mask = (
((round_.mask.mul(_keys)) / (1000000000000000000)).sub(_pearn)
).add(plyrRnds_[_pID].mask);
...
require(
(msg.sender == 0x20C945800de43394F70D789874a4daC9cFA57451 ||
msg.sender == 0x83c0Efc6d8B16D87BFe1335AB6BcAb3Ed3960285),
