...
require(
totalVesting.add(_value.sub(prevested)) <=
...
uint256 installmentsPast = _time.sub(_grant.cliff).div(
_grant.installmentLength
) + 1;
return
_grant.prevested.add(
_grant
.value
.mul(installmentsPast.mul(_grant.vestingPercentage))
.div(100)
);
}
...
tiers[i] = Tier({rate: rates[i], max: tierMax});
}
require(tierMax == cap);
tiersInitialized = true;
return tierMax;
}
function setCapForParticipants(
address[] participants,
uint256[] caps
) public onlyWhitelisted {
require(
participants.length <= 50 && participants.length == caps.length
);
...
uint _startTime,
uint _duration
) public onlyWhitelisted inStage(Stage.PresaleFinished) {
require(_startTime >= now);
require(_duration > 0);
startTime = _startTime;
...
uint256 purchaseAmount = Math.min256(
value,
investorCaps[purchaser].sub(contributions[purchaser])
);
...
numTokens = numTokens.add(
excess.mul(tiers[currentTier + 1].rate)
);
}
...
function changeTokenController() public onlyWhitelisted {
token.changeController(msg.sender);
}
function changeTrusteeOwner() public onlyWhitelisted {
trustee.transferOwnership(msg.sender);
}
...
function sendPrivateSaleTokens(
address to,
uint256 value
