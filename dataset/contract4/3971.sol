...
address _owner,
address _spender
) public view returns (bool _success, uint256 _remaining) {}
function balanceOf(
address _owner
) public view returns (bool _success, uint256 _balance) {}
}
...
address _owner,
address _spender
) public view returns (uint256 _remaining) {}
...
function changeOffchainUploaderAddress(
address _offchainUploaderAddress
) external forOwner {
offchainUploaderAddress = _offchainUploaderAddress;
}
...
address _owner
) public view returns (bool _success, bool _allow) {
address _trg = libAddress;
assembly {
let m := mload(0x40)
calldatacopy(m, 0, calldatasize)
...
uint256 _input
) public view returns (bool _success, uint256 _reward) {
address _trg = libAddress;
assembly {
let m := mload(0x40)
calldatacopy(m, 0, calldatasize)
...
thisBalance = thisBalance.add(
vesting[_beneficiary].amount.sub(
vesting[_beneficiary].claimedAmount
)
);
if (_amount == 0) {
...
vesting[msg.sender].claimedAmount = vesting[msg.sender]
.claimedAmount
.add(_reward);
...
require(
currentPhase != phaseType.pause && currentPhase != phaseType.finish
);
...
bool _subResult;
require(
currentPhase == phaseType.privateSale2 ||
...
true,
_owner == address(this) ||
transferRight[_owner] ||
currentPhase == phaseType.preFinish ||
currentPhase == phaseType.finish
);
}
function calculateReward(
uint256 _input
...
_amount = _input.mul(1e8).mul(100).mul(currentRate).div(1e18).div(
currentRateM
);
if (_amount == 0) {
return;
...
_reward = _vesting
.amount
.mul(block.number.sub(_vesting.startBlock))
.div(_vesting.endBlock.sub(_vesting.startBlock));
