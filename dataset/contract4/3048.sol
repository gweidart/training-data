...
string public constant name = "FoMo3D Chicken";
string public constant symbol = "FomoC";
uint256 private rndExtra_ = 0;
uint256 private constant rndInit_ = 72 hours;
uint256 private constant rndInc_ = 30 seconds;
uint256 private constant rndMax_ = 72 hours;
uint256 public rID_;
...
F3Ddatasets.EventReturns memory _eventData_ = determinePID(_eventData_);
uint256 _pID = pIDxAddr_[msg.sender];
if (plyr_[_affCode].addr == address(0)) _affCode = 0;
...
uint256 _pID = pIDxAddr_[msg.sender];
uint256 _affID;
if (pIDxAddr_[_affCode] == 0) _affID = 0;
...
uint256 _pID = pIDxAddr_[msg.sender];
uint256 _affID;
if (pIDxAddr_[_affCode] == 0) _affCode = address(0);
if (_affCode == address(0) || _affCode == msg.sender) {
_affID = plyr_[_pID].laff;
} else {
_affID = pIDxAddr_[_affCode];
...
function withdrawComB() public isActivated isHuman {
require(
...
if (_eth > 1000000000) {
uint256 _keys = (_eth.mul(1000000000000000000) /
round_[_rID].keyPrice);
if (_keys >= 1000000000000000000) {
round_[_rID].keyPrice = round_[_rID].keyPrice.add(
10000000000000
);
