...
function approveMintDelegate(
address _mintDelegate
...
function approveBurnDelegate(
address _burnDelegate
) public onlyOwner returns (bool) {
bool delegateFound = false;
for (uint i = 0; i < burnDelegates.length; i++) {
...
function mint(
uint256 _amount
) public onlyOwnerOrMintDelegate returns (bool) {
...
function burn(
uint256 _value
) public onlyOwnerOrBurnDelegate returns (bool) {
require(_value <= balances[msg.sender]);
