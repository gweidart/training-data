...
balances[token][msg.sender] = safeAdd(
...
balances[address(0)][msg.sender] = safeAdd(
balances[address(0)][msg.sender],
msg.value
);
Deposit(
address(0),
...
withdraws[hash] = true;
balances[token][msg.sender] = safeSub(
balances[token][msg.sender],
amount
);
if (token == address(0)) {
...
uint8[3] v,
bytes32[6] rs
) public onlyNotPaused returns (bool) {
...
balances[addrs[1]][addrs[2]] = safeSub(
balances[addrs[1]][addrs[2]],
vals[7]
);
require(balances[addrs[0]][addrs[3]] >= vals[6]);
balances[addrs[0]][addrs[3]] = safeSub(
balances[addrs[0]][addrs[3]],
vals[6]
);
balances[addrs[0]][addrs[2]] = safeAdd(
balances[addrs[0]][addrs[2]],
safeSub(vals[6], (safeMul(vals[6], vals[9]) / 1 ether))
);
balances[addrs[1]][addrs[3]] = safeAdd(
balances[addrs[1]][addrs[3]],
safeSub(vals[7], (safeMul(vals[7], vals[10]) / 1 ether))
);
balances[addrs[0]][feeAccount] = safeAdd(
balances[addrs[0]][feeAccount],
safeMul(vals[6], vals[9]) / 1 ether
);
balances[addrs[1]][feeAccount] = safeAdd(
balances[addrs[1]][feeAccount],
safeMul(vals[7], vals[10]) / 1 ether
);
...
uint256 minedBEY = safeMul(
safeMul(vals[7], getMiningRate(addrs[1])),
2
) / 1 ether;
if (unmined > minedBEY) {
...
mined[addrs[3]] = safeAdd(
mined[addrs[3]],
safeSub(unmined, unmined / 2)
);
unmined = 0;
}
}
