...
function getCurrentTokenPricepreICO(
uint256 individuallyContributedEtherInWei
) private returns (uint) {
require(
individuallyContributedEtherInWei >=
(minContribution.mul(ETH_DECIMALS))
);
uint disc;
bonusCalculationFactor = (block.timestamp.sub(preico_startdate)).div(
604800
);
if (bonusCalculationFactor == 0) disc = 30;
else if (bonusCalculationFactor == 1) disc = 20;
else if (bonusCalculationFactor == 2) disc = 10;
else if (bonusCalculationFactor == 3) disc = 5;
return disc;
}
function getCurrentTokenPriceICO(
uint256 individuallyContributedEtherInWei
) private returns (uint) {
require(
individuallyContributedEtherInWei >=
...
uint disc;
bonusCalculationFactor = (block.timestamp.sub(ico_startdate)).div(
604800
);
if (bonusCalculationFactor == 0) disc = 30;
else if (bonusCalculationFactor == 1) disc = 20;
...
else if (bonusCalculationFactor > 3) disc = 0;
return disc;
}
function y() private {
no_of_tokens = ((msg.value).mul(priceFactor.mul(1000))).div(
_price_token
);
