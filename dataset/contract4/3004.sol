...
event LogFundTransfer(
address indexed Backer,
uint indexed Amount,
bool indexed IsContribution
);
function BOSTokenCrowdfund(token _addressOfTokenUsedAsReward) {
tokenReward = token(_addressOfTokenUsedAsReward);
}
function() payable {
require(!crowdsaleClosed);
require(msg.value > 0);
uint tokens = SafeMath.safeMul(
SafeMath.safeDiv(msg.value, sellPrice),
1 ether
);
...
function publicWithdraw() public onlyAllowPublicWithdraw {
...
function withdrawAmountTo(uint256 _amount, address _to) public onlyOwner {
