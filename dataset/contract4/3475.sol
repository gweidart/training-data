...
uint internal constant MAX_RATE = (PRECISION * 10 ** 6);
uint internal constant MAX_DECIMALS = 18;
...
uint internal constant MAX_GROUP_SIZE = 50;
...
PermissionGroups _wrappedContract,
address _admin,
uint _numDataInstances
) public Withdrawable() {
...
uint signedNonce,
address signer
) internal returns (bool allSigned) {
require(dataIndex < dataInstances.length);
require(dataInstances[dataIndex].lastSetNonce == signedNonce);
for (
uint i = 0;
i < dataInstances[dataIndex].approveSignatureArray.length;
i++
) {
if (signer == dataInstances[dataIndex].approveSignatureArray[i])
revert();
}
dataInstances[dataIndex].approveSignatureArray.push(signer);
if (
dataInstances[dataIndex].approveSignatureArray.length ==
operatorsGroup.length
) {
allSigned = true;
} else {
allSigned = false;
...
function getDataTrackingParameters(
uint index
) internal view returns (address[], uint) {
require(index < dataInstances.length);
return (
dataInstances[index].approveSignatureArray,
dataInstances[index].lastSetNonce
);
}
}
...
function WrapFeeBurner(
FeeBurner feeBurner,
address _admin
)
public
WrapperBase(
PermissionGroups(address(feeBurner)),
_admin,
LAST_DATA_INDEX
)
{
...
function setPendingKNCRateRange(
uint minRate,
uint maxRate
) public onlyOperator {
...
public
view
returns (uint minRate, uint maxRate, uint nonce)
{
...
function setPendingReserveData(
address reserve,
uint feeBps,
address kncWallet
) public onlyOperator {
...
feeBurnerContract.setReserveData(
addReserve.reserve,
addReserve.feeBps,
addReserve.kncWallet
);
...
public
view
returns (address wallet, uint feeBps, uint nonce)
{
address[] memory signatures;
...
feeBurnerContract.setWalletFees(
walletFee.walletAddress,
walletFee.feeBps
);
}
}
function setPendingTaxParameters(
address taxWallet,
uint feeBps
) public onlyOperator {
