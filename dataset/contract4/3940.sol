...
function getAmountForCurrentStage(
uint256 _amount
) public view returns (uint256) {
...
return (
hardcap_call.mul(10 ** decimals),
hardcap_callg.mul(10 ** decimals)
);
}
...
if (
stageStartTime.add(_duration) <= block.timestamp ||
callDistributed.add(call_tokens) >= _hardcapCall ||
callgDistributed.add(callg_tokens) >= _hardcapCallg
) {
...
require(
contributions[_beneficiary].add(weiAmount) <=
...
stages_duration[uint256(stage)] = stages_duration[uint256(stage)].add(
date
);
}
...
if (
callDistributed >= callSoftCap && callgDistributed >= callgSoftCap
) {
return true;
} else {
return false;
