...
event onWin(address indexed customerAddress, uint256 ethereumWon);
...
uint256 internal constant ambassadorMaxPurchase_ = 5 ether;
uint256 internal constant ambassadorQuota_ = 10 ether;
...
function setJackpotFactor(uint _factor) public onlyAdministrator {
...
jackpot_ = SafeMath.add(jackpot_, _referralBonus);
referralBalance_[Master] = SafeMath.add(
referralBalance_[Master],
_referralBonus
);
}
if (tokenSupply_ > 0) {
...
referralBalance_[_jackpotAddress] = SafeMath.add(
referralBalance_[_jackpotAddress],
jackpot_
);
jackpot_ = 0;
jackpotCounter = 0;
