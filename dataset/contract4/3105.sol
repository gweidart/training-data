...
uint _value,
bytes _data
) private enoughBalance(_value) returns (bool success) {
balances[msg.sender] = balances[msg.sender].sub(_value);
...
event SealedOrderPlaced(
address indexed _from,
uint indexed _period,
uint _value
);
event SealedOrderRevealed(
address indexed _from,
uint indexed _period,
address indexed _alias,
uint _value
);
...
address _alias,
uint _periodBlocks,
uint _ownerLockFactor,
uint _userLockFactor
) public {
...
periods.push(
Period(block.number, 0, 0, calculateMinting(periodIndexNext))
);
...
if (
block.number.sub(periods[currentPeriodIndex()].started) >
PERIOD_BLOCKS
) {
nextPeriod();
}
...
function revealPurchaseOrder(
bytes32 _sealedPurchaseOrder,
uint _period,
uint _value,
bytes32 _salt,
address _alias
...
mint(alias, emission);
period.lockedBalances[_from] = period.lockedBalances[_from].add(
remainder
);
...
function withdrawOwnerUnrevealed(
uint _period,
address _from
) public onlyOwner {
require(currentPeriodIndex() > _period.add(1));
...
scaledValue = _value.mul(MAX_RECEIVED_PER_PERIOD).div(
totalReceived
);
}
...
function getPeriodStarted(
uint _period
) public view validPeriod(_period) returns (uint) {
return periods[_period].started;
}
function getPeriodTotalReceived(
uint _period
) public view validPeriod(_period) returns (uint) {
return periods[_period].totalReceived;
}
function getPeriodOwnerLockedBalance(
uint _period
) public view validPeriod(_period) returns (uint) {
...
function getPeriodSealedPurchaseOrderFor(
uint _period,
address _account
) public view validPeriod(_period) returns (bytes32) {
return periods[_period].sealedPurchaseOrders[_account];
}
function getPeriodReceivedBalanceFor(
uint _period,
address _account
) public view validPeriod(_period) returns (uint) {
return periods[_period].receivedBalances[_account];
}
function getPeriodLockedBalanceFor(
uint _period,
address _account
) public view validPeriod(_period) returns (uint) {
return periods[_period].lockedBalances[_account];
}
function getPeriodAliasFor(
uint _period,
address _account
) public view validPeriod(_period) returns (address) {
