...
uint256 a,
uint256 b
) public pure returns (bool isTrue) {
isTrue = a < b;
return isTrue;
}
...
function buyPack(uint8 _editionNumber) public payable {
uint256 packPrice = isVIP[msg.sender]
? 0
: editions[_editionNumber].packPrice;
...
_processERC20TokenPackPurchase(
_editionNumber,
_tokenAddress,
msg.sender
);
...
require(
cardDamageUpgradeLevel[_cardId].isLessThan(maxCardUpgradeLevel)
);
uint256 costOfUpgrade = 32 * (cardDamageUpgradeLevel[_cardId] + 1);
MythereumERC20Token mythexContract = MythereumERC20Token(
mythexTokenAddress
);
...
require(
cardShieldUpgradeLevel[_cardId].isLessThan(maxCardUpgradeLevel)
);
...
require(
edition.sales.isLessThan(edition.maxSales.plus(edition.packSize))
);
