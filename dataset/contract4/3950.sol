...
uint256 public constant INITIAL_SUPPLY = 17500000E18;
uint256 public constant lockPeriodStart = 1536796740;
...
if (now < unlockTimes[msg.sender]) revert();
}
_;
}
constructor() public {
...
address _to,
uint256 _value
) public isRunning isNotLocked returns (bool) {
...
uint256 public constant startPresale = 1532304000;
uint256 public constant endPresale = 1534118340;
uint256 public constant startSale = 1534118400;
uint256 public constant endSale = 1536796740;
...
function issueTokensPresale(
address _addr,
uint256 _val
) public onlyOwner isAfterPresale isAboveSoftCap {
require(_val >= 0);
...
function issueTokensSale(
address _addr,
uint256 _val
) public onlyOwner isAfterSale isAboveSoftCap {
require(_val >= 0);
require(!tokensaleFinished);
...
function refundTokensPresale(
address[] _addrs
) public onlyOwner isAfterPresale {
for (uint256 i; i < _addrs.length; i++) {
address _addr = _addrs[i];
uint256 _fundedEth = preBalances[_addr];
if (_fundedEth > 0) {
...
uint256 _tokensToBurn = token.balanceOf(this);
