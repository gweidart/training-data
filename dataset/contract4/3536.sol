...
string public constant name = "ethfomo3d";
string public constant symbol = "ethfomo3d";
...
uint256 private constant preIcoMax_ = 50000000000000000000;
uint256 private constant preIcoPerEth_ = 1500000000000000000;
...
mapping(uint256 => mapping(uint256 => FFFdatasets.PlayerRounds))
public plyrRnds_;
...
constructor(
PlayerBookInterface _PlayerBook,
address _yyyy,
address _gggg
) public {
...
if (
round_[_rID].eth < preIcoMax_ &&
plyrRnds_[_pID][_rID].eth.add(_eth) > preIcoPerEth_
) {
uint256 _availableLimit = (preIcoPerEth_).sub(
plyrRnds_[_pID][_rID].eth
);
...
FFFdatasets.EventReturns memory _eventData_
) private returns (FFFdatasets.EventReturns) {
uint256 _pID = pIDxAddr_[msg.sender];
if (_pID == 0) {
...
emit FFFevents.onAffiliatePayout(
_affID,
plyr_[_affID].addr,
plyr_[_affID].name,
_rID,
