...
PlayerBookInterface(0xc6645d6c197f2d634822f08da5809065bad03785);
string public constant name = "HX";
string public constant symbol = "HX";
uint256 private rndExtra_ = 0;
uint256 private rndGap_ = 0;
uint256 private constant rndInit_ = 30 minutes;
...
mapping(uint256 => mapping(uint256 => HXdatasets.PlayerRounds))
public plyrRnds_;
...
HXdatasets.EventReturns memory _eventData_
) private returns (HXdatasets.EventReturns) {
uint256 _pID = pIDxAddr_[msg.sender];
if (_pID == 0) {
...
emit HXevents.onAffiliatePayout(
_affID,
plyr_[_affID].addr,
plyr_[_affID].name,
_rID,
...
require(msg.sender == community_addr, "only community can activate");
...
1406247070314025878906250000000000000000000000000000000000000000
)
).sqrt()
).sub(37499960937500000000000000000000)
) / (78125000);
}
function eth(uint256 _keys) internal pure returns (uint256) {
return
(
(39062500).mul(_keys.sq()).add(
((74999921875000).mul(_keys.mul(1000000000000000000))) / (2)
)
