...
"Need price oracle contract address"
);
...
function() external payable onlyKYCVerifiedInvestor {
...
if (
_cents >= amountBonuses[i].amountFrom &&
...
function delAdmin(address addr) public onlyAdmin {
removeRole(addr, ROLE_ADMIN);
}
function addBackend(address addr) public onlyAdmin {
addRole(addr, ROLE_BACKEND);
}
function delBackend(address addr) public onlyAdmin {
removeRole(addr, ROLE_BACKEND);
}
function addKycVerifiedInvestor(address addr) public onlyBackend {
addRole(addr, ROLE_KYC_VERIFIED_INVESTOR);
}
function delKycVerifiedInvestor(address addr) public onlyBackend {
