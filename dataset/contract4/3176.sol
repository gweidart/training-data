...
require(
!paused &&
(msg.sender == owner || contractAccess[msg.sender] == true)
);
_;
}
...
address _to,
uint256 _amount
) public onlyAccessByGame canMint returns (bool) {
totalSupply_ = totalSupply_.add(_amount);
balances[_to] = balances[_to].add(_amount);
emit Mint(_to, _amount);
emit Transfer(address(0), _to, _amount);
...
string public constant DEFAULT_NAME = "NONAME";
User[] private users;
...
string _name
) public onlyAccessByGame whenNotPaused returns (bool) {
require(_beneficiary != address(0));
require(ownerToUser[_beneficiary] == 0);
User memory user = User({name: _name, registeredDate: uint32(now)});
uint256 id = users.push(user) - 1;
ownerToUser[_beneficiary] = id;
userCount++;
return true;
}
function setName(string _name) public whenNotPaused returns (bool) {
...
function getUserid(
address _owner
) external view onlyAccessByGame returns (uint256 result) {
if (ownerToUser[_owner] == 0) {
return 0;
}
return ownerToUser[_owner];
}
function getUserInfo() public view returns (uint256, string, uint32) {
uint256 userid = ownerToUser[msg.sender];
return getUserInfoById(userid);
}
function getUserInfoById(
uint256 _userid
) public view returns (uint256, string, uint32) {
