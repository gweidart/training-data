...
function spendFromMultisig(
bytes32 msigId,
uint amount,
address recipient
) internal {
multisigs[msigId].deposit = sub(multisigs[msigId].deposit, amount);
if (multisigs[msigId].deposit == 0) delete multisigs[msigId];
recipient.transfer(amount);
}
function initialiseMultisig(
address authority,
uint unlockTime
) public payable returns (bytes32 msigId) {
require(msg.sender != authority);
require(msg.value > 0);
msigId = keccak256(msg.sender, authority, msg.value, unlockTime);
...
function reparametrizeMultisig(
bytes32 msigId,
uint unlockTime
) public payable {
...
function earlyResolve(bytes32 msigId, uint amount, bytes sig) public {
require(
...
function batchRegularTransfers(
bytes32[] swapIds,
bytes32[] secrets
) public {
...
function reclaimExpiredSwap(bytes32 msigId, bytes32 swapId) public {
require(
multisigs[msigId].owner == msg.sender ||
multisigs[msigId].authority == msg.sender
);
