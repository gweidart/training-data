...
address _to,
uint256 _value
) public shouldPassPolicy(msg.sender, _to) returns (bool) {
return super.transfer(_to, _value);
}
function transferFrom(
address _from,
address _to,
uint256 _value
) public shouldPassPolicy(_from, _to) returns (bool) {
return super.transferFrom(_from, _to, _value);
}
function unfrezee() public onlyOwner returns (bool) {
require(!unfrozen);
unfrozen = true;
}
}
...
uint _decimals,
address _registry
) public ApproveAndCallToken() MintableToken() TokenPolicy(_registry) {
name = _name;
ticker = _ticker;
decimals = _decimals;
}
...
require(
userRegistry.knownAddress(_holder) &&
!userRegistry.hasIdentity(_holder)
);
...
function AltToken(
address _registry
) public DefaultToken("AltEstate token", "ALT", 18, _registry) {}
}