...
uint256 _ICO_startingTime,
uint256 _ICO_closingTime
) public isOwner returns (bool success) {
...
function admin_transfer_tempLockAddress(
address _to,
uint256 _value,
uint256 _unlockTime
) public isOwner returns (bool success) {
...
function admin_renewLockedAddress(
address _address,
uint256 _unlockTime
) public isOwner returns (bool success) {
...
require(
ICO_startingTime < block.timestamp &&
ICO_closingTime > block.timestamp
);
require(!depositLock);
uint256 tokenValue;
