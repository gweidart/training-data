...
modifier notLocked() {
require(
msg.sender == owner || msg.sender == saleAgent || mintingFinished
);
_;
}
...
require(
(msg.sender == saleAgent || msg.sender == owner) && !mintingFinished
);
...
address to,
uint investedWei
) public onlyDirectMintAgentOrOwner saleIsOn {
calculateAndMintTokens(to, investedWei);
TokenPurchased(to, investedWei, now);
}
function createTokens() public payable whenNotPaused {
require(msg.value >= minInvestment);
uint masterValue = msg
.value
.mul(percentRate.sub(slaveWalletPercent))
.div(percentRate);
...
investedAmountOf[msg.sender] = investedAmountOf[msg.sender].add(
weiInvested
);
...
function retrieveExternalTokens(
...
address _address
) public onlyDirectMintAgentOrOwner {
...
function deleteAddressesFromWhitelist(
address[] _addresses
) public onlyDirectMintAgentOrOwner {
for (uint256 i = 0; i < _addresses.length; i++) {
...
require(
locked[addr] < now &&
(msg.sender == saleAgent || msg.sender == addr)
);
...
uint newBountyTokensPercent
) public onlyOwner {
bountyTokensPercent = newBountyTokensPercent;
}
function setReservedTokensPercent(
uint newReservedTokensPercent
) public onlyOwner {
...
address newBountyTokensWallet
) public onlyOwner {
bountyTokensWallet = newBountyTokensWallet;
}
function setReservedTokensWallet(
address newReservedTokensWallet
) public onlyOwner {
reservedTokensWallet = newReservedTokensWallet;
}
function endSale() public whenNotPaused saleIsFinished onlyOwner {
uint foundersTokens = totalTokenSupply.mul(teamTokensPercent).div(
percentRate
);
uint reservedTokens = totalTokenSupply.mul(reservedTokensPercent).div(
percentRate
);
uint bountyTokens = totalTokenSupply.mul(bountyTokensPercent).div(
percentRate
);
...
mintTokens(
reservedTokensWallet,
totalTokenSupply.sub(currentSupply)
);
}
...
function() external payable onlyIfWhitelisted {
