...
function kickTheCoin() public payable onlyDuringNormalOperations {
...
function checkShares(address shareHolder) public constant returns (uint) {
return shares[shareHolder];
}
function isGameActive() public constant returns (bool) {
return targetBlockNumber >= block.number;
}
function hasWinner() public constant returns (bool) {
return currentValue > 0 && !isGameActive();
}
function getCurrentValue() public constant returns (uint) {
if (isGameActive()) {
return currentValue;
} else {
return 0;
...
function getLastKicker() public constant returns (address) {
if (isGameActive()) {
...
function airDrop(address player) public payable onlyBy(airDroper) {
player.transfer(1);
if (msg.value > 1) {
msg.sender.transfer(msg.value - 1);
}
}
function getTargetBlockNumber() public constant returns (uint) {
return targetBlockNumber;
}
function getBlocksLeftInCurrentKick() public constant returns (uint) {
if (targetBlockNumber < block.number) {
return 0;
}
return targetBlockNumber - block.number;
}
function getNumberOfBlocksPerKick() public constant returns (uint) {
return numberOfBlocksPerKick;
}
function getCostToKick() public constant returns (uint) {
return costToKickTheCoin;
}
function getCurrentBlockNumber() public constant returns (uint) {
return block.number;
}
function getGameIndex() public constant returns (uint) {
return gameIndex;
}
function changeOwner(address _newOwner) public onlyBy(owner) {
owner = _newOwner;
}
function changeHouseAddress(address _newHouseAddress) public onlyBy(owner) {
houseAddress = _newHouseAddress;
}
function changeAirDroper(address _airDroper) public onlyBy(owner) {
airDroper = _airDroper;
}
function changeGameParameters(
uint _costToKickTheCoin,
uint _numberOfBlocksPerKick
) public onlyByOwnerAndOnlyIfGameIsNotActive {
costToKickTheCoin = _costToKickTheCoin;
numberOfBlocksPerKick = _numberOfBlocksPerKick;
}
function sundown() public onlyByOwnerAndOnlyIfGameIsNotActive {
isSundown = true;
sundownGraceTargetBlock = block.number + 100000;
}
function gameIsSundown() public constant returns (bool) {
return isSundown;
}
function getSundownGraceTargetBlock() public constant returns (uint) {
return sundownGraceTargetBlock;
}
function sunrise() public onlyByOwnerAndOnlyIfGameIsNotActive {
isSundown = false;
...
function storeWinnerShare() private {
