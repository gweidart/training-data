...
event NewRound(uint _timestamp, uint _round, uint _initialPot);
event Bid(uint _timestamp, address _address, uint _amount, uint _newPot);
event NewLeader(
uint _timestamp,
address _address,
uint _newPot,
...
event EarningsWithdrawal(uint _timestamp, address _address, uint _amount);
...
uint _durationDecrease = DURATION_DECREASE_PER_ETHER.mul(
pot.div(1 ether)
);
uint _duration;
...
uint _minLeaderAmount = pot.mul(MIN_LEADER_FRAC_TOP).div(
MIN_LEADER_FRAC_BOT
);
uint _bidAmountToCommunity = msg.value.mul(FRAC_TOP).div(FRAC_BOT);
uint _bidAmountToDividendFund = msg
.value
.mul(DIVIDEND_FUND_FRAC_TOP)
.div(DIVIDEND_FUND_FRAC_BOT);
uint _bidAmountToPot = msg.value.sub(_bidAmountToCommunity).sub(
_bidAmountToDividendFund
);
...
dividendShares[msg.sender] = dividendShares[msg.sender].add(
_dividendShares
);
...
uint _amount = dividendFund.mul(_dividendShares).div(
totalDividendShares
);
