...
currentCorpBank_.deposit.value(address(this).balance)(
address(currentCorpBank_)
);
}
...
require(
needsBank_ == false,
"Forwarder Deposit failed - no registered bank"
);
...
require(
msg.sender == address(currentCorpBank_),
"Forwarder startMigration failed - msg.sender must be current corp bank"
);
if (
ProForwarderInterface(_newCorpBank).migrationReceiver_setup() ==
true
) {
