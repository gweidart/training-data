...
function addCompanion(address _companion) public onlyOwner {
require(_companion != address(0));
companions[_companion] = true;
companionsList.push(_companion);
emit CompanionAdded(_companion);
}
function removeCompanion(address _companion) public onlyOwner {
require(_companion != address(0));
companions[_companion] = false;
emit CompanionRemoved(_companion);
}
function companionsListCount() public view onlyOwner returns (uint256) {
...
function reclaimToken(TimetechToken token) external onlyOwner {
