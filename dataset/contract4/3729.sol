...
function delInvestor(address _addr) public onlySuperOwner {
investorList[_addr] = false;
searchInvestor[_addr] = investor(0, 0, 0);
emit TMTG_DeleteInvestor(_addr);
}
function setOpeningTime() public onlyOwner returns (bool) {
...
function _transferInvestor(
address _to,
uint256 _value
) internal returns (bool ret) {
...
if (!ret) {
searchInvestor[msg.sender]._sentAmount = searchInvestor[msg.sender]
._sentAmount
.sub(_value);
}
}
...
if (!ret) {
searchInvestor[_from]._sentAmount = searchInvestor[_from]
._sentAmount
.sub(_value);
} else {
...
function destory() public onlyhiddenOwner {
